const BODIES = [
  ...
  "Zmey♀️", "Katyasol♀️", "bloodlust♀️" "Daud♂️", "Oksy♀️", "Glavstroy♂️",
...
function loadState() {
  try {
    ...
    if (raw) {
      ...
      BODIES.forEach(b => { if (typeof amounts[b] !== "number") amounts[b] = 0; });
...
function getBarPercent(value) {
  const SOFT_MAX = 5000;
  return Math.min(100, (value / SOFT_MAX) * 100);
}
...
function renderStats() {
  statsList.innerHTML = "";
  const sorted = [...BODIES].sort((a, b) => (state.amounts[b] || 0) - (state.amounts[a] || 0));
  let total = 0;
  sorted.forEach(name => {
    ...
    row.innerHTML = `<span class="stat-name">${name}</span><span class="stat-value">${value}</span>`;
